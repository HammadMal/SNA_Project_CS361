...
# Script: Candidate-Candidate Network Metrics Visualizations
# ==============================================================================
...
cat("================================================================================\n")
cat("CANDIDATE-CANDIDATE NETWORK METRICS VISUALIZATIONS\n")
cat("================================================================================\n\n")
...
# Output directory
output_dir <- file.path(resultpath, "network_metrics_visualizations_candidate")
...
# --- Plot 1: Top 30 Candidates ---
top_30_deg <- head(centrality[order(-centrality$Degree), ], 30)

barplot(top_30_deg$Degree,
        names.arg = top_30_deg$Candidate_ID,
        main = "Top 30 Candidates by Degree Centrality",
...
hist(centrality$Degree,
     ...
     xlab = "Degree",
     ylab = "Frequency (Number of Candidates)",
     col = "lightblue",
     border = "white",
     cex.axis = 1.0,
     cex.lab = 1.2,
...
barplot(top_30_betw$Betweenness,
        names.arg = top_30_betw$Candidate_ID,
        main = "Top 30 Candidates by Betweenness Centrality",
        ylab = "Betweenness",
        col = "coral",
        las = 2,
        cex.names = 0.7,
...
barplot(top_30_clos$Closeness,
        names.arg = top_30_clos$Candidate_ID,
        main = "Top 30 Candidates by Closeness Centrality",
        ylab = "Closeness",
        col = "gold",
        las = 2,
        cex.names = 0.7,
...
legend("topright",
       legend = c(sprintf("Mean = %.6f", mean(centrality$Closeness)),
                 sprintf("Median = %.6f", median(centrality$Closeness))),
       col = c("red", "black"),
       lty = c(2, 0),
       lwd = c(3, 0),
       cex = 1.1,
...
text(1.3, mean(centrality$Closeness),
     sprintf("Mean = %.6f", mean(centrality$Closeness)),
     cex = 1.1)
...
barplot(top_30_eigen$Eigenvector,
        names.arg = top_30_eigen$Candidate_ID,
        main = "Top 30 Candidates by Eigenvector Centrality",
        ylab = "Eigenvector",
        col = "lightgreen",
        las = 2,
        cex.names = 0.7,
...
legend("topright",
       legend = c(sprintf("Mean = %.6f", mean(centrality$Eigenvector)),
                 sprintf("Median = %.6f", median(centrality$Eigenvector))),
       col = c("red", "black"),
       lty = c(2, 0),
       lwd = c(3, 0),
       cex = 1.1,
...
text(1.3, mean(centrality$Eigenvector),
     sprintf("Mean = %.6f", mean(centrality$Eigenvector)),
     cex = 1.1)
...
barplot(top_30_pr$PageRank,
        names.arg = top_30_pr$Candidate_ID,
        main = "Top 30 Candidates by PageRank Centrality",
        ylab = "PageRank",
        col = "mediumpurple",
        las = 2,
        cex.names = 0.7,
...
legend("topright",
       legend = c(sprintf("Mean = %.6f", mean(centrality$PageRank)),
                 sprintf("Median = %.6f", median(centrality$PageRank))),
       col = c("red", "black"),
       lty = c(2, 0),
       lwd = c(3, 0),
       cex = 1.1,
...
text(1.3, mean(centrality$PageRank),
     sprintf("Mean = %.6f", mean(centrality$PageRank)),
     cex = 1.1)
...
# --- Plot 1: Top 30 Candidates (Lowest Eccentricity = Most Central) ---
top_30_ecc <- head(centrality[order(centrality$Eccentricity), ], 30)

barplot(top_30_ecc$Eccentricity,
        names.arg = top_30_ecc$Candidate_ID,
        main = "Top 30 Candidates by Eccentricity (Lowest = Most Central)",
        ylab = "Eccentricity",
        col = "skyblue",
        las = 2,
        cex.names = 0.7,
...
local_clustering <- transitivity(g_candidate, type = "local")
...
barplot(top_30_clust$Clustering,
        names.arg = top_30_clust$Candidate_ID,
        main = "Top 30 Candidates by Clustering Coefficient",
...
barplot(comp_sizes_plot,
        ...
        xlab = "Component Rank",
        ylab = "Size (Number of Candidates)",
        col = "lightsalmon",
        cex.names = 0.9,
        cex.axis = 1.1,
        cex.lab = 1.2,
...
# Add overall title
mtext("Basic Network Metrics Summary - Candidate Network", 
      outer = TRUE, 
      cex = 1.5, 
      font = 2,
      line = 0.5)
...
pdf(file.path(output_dir, "09_centrality_comparison_comprehensive.pdf"), 
    width = 18, height = 14)

# Get top 20 candidates overall
...
cent_matrix <- as.matrix(top_20_overall[, c("Degree_norm", "Betweenness_norm",
                                              ...
                                              "PageRank_norm")])
rownames(cent_matrix) <- top_20_overall$Candidate_ID

barplot(t(cent_matrix),
        beside = TRUE,
        main = "Top 20 Candidates - All Centrality Measures (Normalized)",
        xlab = "",
...
dev.off()
cat("✓ Saved: 09_centrality_comparison_comprehensive.pdf\n\n")
...
cat("  • 09_centrality_comparison_comprehensive.pdf (1 plot)\n\n")

cat("CENTRALITY PLOTS (01-07) EACH CONTAIN:\n")
cat("  1. Top 30 candidates bar chart (NO OVERLAP)\n")
cat("  2. Distribution histogram\n")
cat("  3. Boxplot with statistics\n")
cat("  4. Scatter plot vs Degree\n\n")

cat("BASIC METRICS PLOTS (08) CONTAIN:\n")
...
cat("✓ Total: 9 PDFs with 33 total plots\n")
